...
const Map = struct {
    ...
    fn get(self: *const Self, pos: Vec2) ?u8 {
        if (pos.x >= self.x_len or pos.y >= self.y_len or pos.x < 0 or pos.y < 0) return null;
...
fn update_input(input: []u8, pos: Vec2, ch: u8) void {
    const p = pos.cast(usize);
    var iter = std.mem.splitSequence(u8, input, "\n");
    const x_len = iter.next().?.len;

    const idx = (x_len+1)*p.y + p.x;
    input[idx] = ch;
}

const ABool = atomic.ABool;

pub fn find_sol(alloc: std.mem.Allocator, update_rate_ms: u32, input: []u8, info_buf: []u8, done: *ABool) !void {
    var map = blk: {
        var input_iter = std.mem.splitSequence(u8, input, "\n");
        var lines = std.ArrayList([]u8).init(alloc);
        while (input_iter.next()) |l| {
            const line = try alloc.dupe(u8, l);
    ...
    const unique = try find_normal_path(alloc, g, map);
    ...
    defer arena.deinit();
    var obst_idx: u32 = 0;
    u_loop: while (unique_pos_iter.next()) |u| : (obst_idx += 1) {
        defer { _ = arena.reset(.free_all); }

        map.set_obstacle_at(u);
        update_input(input, u.*, OBSTACLE);
        defer {
            map.set_path_at(u);
            update_input(input, u.*, PATH);
            g.pos = orig_pos;
            g.dir = orig_dir;
        }
        ...
        while (true) {
            ...
            const new_obst_info = try std.fmt.bufPrint(info_buf, "\nobstacles_that_loop_count: {d}\nnew_obstacle: {d}, {d}, idx: {d}\n", .{obstacles_that_loop_count, u.x, u.y, obst_idx});
            @memset(info_buf[new_obst_info.len..], ' ');
            const guard_pos_buf = info_buf[new_obst_info.len..];
            ...
            if (map.get(new_pos) == OBSTACLE) {
                ...
                if (loop_started) {
                    if (obstacles_hit.items.len < obstacles_hit_again.items.len) {
                        update_input(input, g.pos, PATH);
                        update_input(input, orig_pos, GUARD);
                        continue :u_loop;
                ...
                g.rotate_90deg();
                delta = g.dir.delta();
                const guard_pos = try std.fmt.bufPrint(guard_pos_buf, "guard_pos: {d}, {d}", .{g.pos.x, g.pos.y});
                @memset(guard_pos_buf[guard_pos.len..], ' ');
                ...
                update_input(input, g.pos, PATH);
                g.pos = new_pos;
                update_input(input, new_pos, g.guard_ch());
                const new_pos_info = try std.fmt.bufPrint(guard_pos_buf, "guard_pos: {d}, {d}", .{g.pos.x, g.pos.y});
                @memset(guard_pos_buf[new_pos_info.len..], ' ');
            }
            std.time.sleep(update_rate_ms * std.time.ns_per_ms);
        }
    }
    const info = try std.fmt.bufPrint(info_buf, "\nnot running\n", .{});
    @memset(info_buf[info.len..], ' ');
...
fn find_guard_position(map: *const Map) ?Vec2 {
    for (0..map.x_len) |idx_x| {
        for (0..map.y_len) |idx_y| {
