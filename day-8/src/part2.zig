...
fn calculate_antinodes(allocator: mem.Allocator, deltas: DeltaMap, map: Map) ![]Pos {
    ...
    while (entry_iter.next()) |e| {
        ...
        const T = Pos.@"type";
        try list.append(combination[0]);
        try list.append(combination[1]);

        // while valid keep adding antinodes in the same direction
        // left direction
        {
            var antinode_a = combination[0].sub_delta(delta);
            while (map.is_antinode_valid(antinode_a)) {
                const pos = antinode_a.cast(T);
                try list.append(pos);
                antinode_a = pos.sub_delta(delta);
            }
        }

        // right direction
        {
            var antinode_b = combination[0].add_delta(delta);
            while (map.is_antinode_valid(antinode_b)) {
                const pos = antinode_b.cast(T);
                try list.append(pos);
                antinode_b = pos.add_delta(delta);
            }
        }
    }
    return list.toOwnedSlice();
